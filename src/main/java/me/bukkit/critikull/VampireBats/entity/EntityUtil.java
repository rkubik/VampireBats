...
public class EntityUtil {
	...
	public static boolean isPlayerHolding(EntityHuman player, Item item, String itemName) {
		...
		if (player.getItemInMainHand() != null) {
			if (player.getItemInMainHand().getItem() == item && (itemName == null || player.getItemInMainHand().getName().equalsIgnoreCase(itemName))) {
				return true;
			}
		}
	
		if (player.getItemInOffHand() != null) {
			if (player.getItemInOffHand().getItem() == item && (itemName == null || player.getItemInOffHand().getName().equalsIgnoreCase(itemName))) {
				return true;
			}
		}
