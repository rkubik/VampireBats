...
public class VampireBat extends EntityBat {
	...
	public void init() {
		if (this.vampire != null) {
			EntityLiving target = this.vampire.getTargetPlayer();
	...
	private void transform() {
		if (aw() < 0.5F) {
			...
			this.vampire.addEffect(new MobEffect(MobEffects.CONFUSION, 200, 0));
		}
	}
	...
	public void B_() {
		...
		if (validPlayerTarget(this.targetPlayer)) {
			if ((MinecraftServer.currentTick - this.lastAttackTick) > 20) {
				BlockPosition block = new BlockPosition(this.targetPlayer.locX, this.targetPlayer.locY,
						this.targetPlayer.locZ);
				if (block.distanceSquared(this.locX, this.locY, this.locZ) <= DAMAGE_RADIUS * DAMAGE_RADIUS) {
					this.targetPlayer.damageEntity(DamageSource.mobAttack(this),
							ATTACK_DAMAGE + this.random.nextFloat());
	...
	private static boolean validPlayerTarget(EntityHuman player) {
		return player != null && player.isAlive() && !player.isCreativeAndOp() && !player.isSpectator()
				&& !EntityUtil.isPlayerHolding(player, Items.POISONOUS_POTATO, "garlic");
	}
	...
	protected void M() {
		...
		if (((MinecraftServer.currentTick - this.lastAttackTick) > MAX_AGGRO_TICKS)
				|| (targetPlayer != null && !validPlayerTarget(targetPlayer))) {
			this.targetPlayer = null;
		}

		if (this.targetPlayer == null) {
			for (Entity entity : EntityUtil.getEntitiesAroundPoint(this.locX, this.locY, this.locZ, this.world,
					AGGRO_RADIUS)) {
		...
		if (isAsleep()) {
			...
			if ((this.targetBlock != null)
					&& ((!this.world.isEmpty(this.targetBlock)) || (this.targetBlock.getY() < 1))) {
				this.targetBlock = null;
			}
			if (targetPlayer != null) {
				this.targetBlock = new BlockPosition(targetPlayer.locX, targetPlayer.locY, targetPlayer.locZ);
			} else if ((this.targetBlock == null) || (this.random.nextInt(30) == 0)
					|| (this.targetBlock.distanceSquared((int) this.locX, (int) this.locY, (int) this.locZ) < 4.0D)) {
				this.targetBlock = new BlockPosition((int) this.locX + this.random.nextInt(7) - this.random.nextInt(7),
			...
			this.bg = 0.5F;
			this.yaw += f2;
			if (targetPlayer == null && (this.random.nextInt(100) == 0)
					&& (this.world.getType(localBlockPosition2).l())) {
				setAsleep(true);
			}
		}
	}
