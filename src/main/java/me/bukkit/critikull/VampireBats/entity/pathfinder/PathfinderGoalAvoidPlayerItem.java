...
public class PathfinderGoalAvoidPlayerItem extends PathfinderGoal {
	private final Predicate<Entity> c = new Predicate<Entity>() {
		...
		public boolean apply(@Nullable Entity paramAnonymousEntity) {
			return (paramAnonymousEntity.isAlive()) && (PathfinderGoalAvoidPlayerItem.this.a.getEntitySenses().a(paramAnonymousEntity)) && (!PathfinderGoalAvoidPlayerItem.this.a.r(paramAnonymousEntity));
		}
	};
	...
	public PathfinderGoalAvoidPlayerItem(EntityCreature paramEntityCreature, Item item, float paramFloat, double paramDouble1, double paramDouble2) {
		this(paramEntityCreature, item, null, paramFloat, paramDouble1, paramDouble2);
	}

	public PathfinderGoalAvoidPlayerItem(EntityCreature paramEntityCreature, Item item, String itemName, float paramFloat, double paramDouble1, double paramDouble2) {
		this(paramEntityCreature, Predicates.alwaysTrue(), item, itemName, paramFloat, paramDouble1, paramDouble2);
	}

	public PathfinderGoalAvoidPlayerItem(EntityCreature paramEntityCreature, Predicate<? super EntityHuman> paramPredicate, Item item, String itemName, float paramFloat, double paramDouble1, double paramDouble2) {
	...
	public boolean a() {
		...
		List<EntityHuman> localList = this.a.world.a(EntityHuman.class, this.a.getBoundingBox().grow(this.f, 3.0D, this.f), Predicates.and(new Predicate[] { IEntitySelector.d, this.c, this.j }));

		if (localList.isEmpty()) {
			return false;
		}

		if (!EntityUtil.isPlayerHolding((EntityHuman)localList.get(0), this.k, this.l)) {
			return false;
		}

		this.b = (EntityHuman)localList.get(0);
